...
def check_stock(url):
    try:
        ...
        # SpringFarma specific selectors first
        for tag in soup.find_all(class_=lambda c: c and any(x in c.lower() for x in ["stock", "availability", "disponibil"])):
            ...
            if "INDISPONIBIL" in tag_text or "OUT OF STOCK" in tag_text or "STOC EPUIZAT" in tag_text:
                return "nostock"
            if "ÎN STOC" in tag_text or "IN STOC" in tag_text or "IN STOCK" in tag_text or "DISPONIBIL" in tag_text:
                return "stock"

        # Fallback: full page text scan
        if "INDISPONIBIL" in text or "OUT OF STOCK" in text or "STOC EPUIZAT" in text:
            return "nostock"
...
def main():
    ...
    for prod in products:
        ...
        # Detect status change from previous day
